...
$(document).ready(function()
{
    ...
    function MatchmakingViewModel()
    {
        ...
        self.matchMakingRequestFailHandler = function(data)
        {
            try
            {
            ...
            else if (data.ErrorCode === "Banned")
            {
                self.handleError({ title: '!LOC:Banned', desc: loc("!LOC:Your account has been banned. If you believe this is in error, contact us at __email__", { email: "support@planetaryannihilation.com" }) });
            }
            else if (data.ErrorCode === "Maintenance")
            {
                self.handleError({ title: '!LOC:Offline for Maintenance', desc: '!LOC:We will be back soon!' });
            }
            else
            {
        ...
        self.matchMakingBegin = function()
        {
            engine.asyncCall('matchmaking.begin', self.uberId(), self.matchMakingRegion(), api.content.activeContent())
                ...
                .fail(self.matchMakingRequestFailHandler);
    ...
    {
        if (data && data.error == 'Removed from queue')
            return;

        api.debug.log("matchmaking_error", JSON.stringify(data, null, 2));
